...
public class Inventario {
        ...
        if(inventario.exists()){
            try {
                ...
                while(txt.hasNextLine()){
                    ...
                    ArrayList<String> tagAux = new ArrayList<String>(aux);
                    for(int i = 2; i < aux;i++){
                        int val = i-2;
                        tagAux.set(i,dadosdoItem[i]) ;
                    }
    ...
    public ArrayList<Item> filtraItem(ArrayList<String> tag, Projeto projeto){
        ArrayList<Item> itensEncontrados = new ArrayList();
        ...
        for(int i = 0; i < tag.size();i++){
            for(int j = 0; j < ItensDesejados.size();j++){
                ...
                ArrayList tagItemEstoque = itemAux.getTags();
                if(tag.get(i).equals(tagItemEstoque.get(j)) && !itensEncontrados.contains(itemAux)){
                    itensEncontrados.add(itemAux);
                }
                if(i == 0 && itemAux.isProjeto()){
                   Projeto projetoAux = itemAux.getProjeto();
